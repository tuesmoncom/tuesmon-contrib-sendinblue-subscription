...
def check_sendinblue_tuesmon_users_list_id(app_configs, **kwargs):
    from django.conf import settings

    users_list_id = getattr(settings, "SENDINBLUE_TUESMON_USERS_LIST_ID", None)
    if users_list_id is not None:
        return []

    return [checks.Error("SENDINBLUE_TUESMON_USERS_LIST_ID must be set on settings",
...
class SendinblueSubscriptionAppConfig(AppConfig):
    name = "tuesmon_contrib_sendinblue_subscription"
    ...
    def ready(self):
        ...
        register(check_sendinblue_tuesmon_users_list_id)
